...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP):
    ...
    self.stopped = False
    self.stoppingdist = self.c_params.get("StoppingDist", return_default=True)
    ...
    self.hdp_use = self.c_params.get_bool("KisaHDPuse")
    self.canfd_debug = self.c_params.get("KisaCanfdDebug", return_default=True)
  ...
  def create_can_msgs(self, apply_steer_req, apply_torque, torque_fault, set_speed_in_units, accel, stopping, hud_control, actuators, CS, CC):
    ...
    if CS.out.cruiseState.standstill:
      ...
      # at least 0.1 sec delay after entering the standstill
        ...
        self.standstill_fault_reduce_timer += 1
      # gap save after 1sec
      elif 100 < self.standstill_fault_reduce_timer and self.cruise_gap_prev == 0 and CS.cruiseGapSet != 1.0 and self.kisa_cruisegap_auto_adj:
        self.cruise_gap_prev = CS.cruiseGapSet
        self.cruise_gap_set_init = True
      # gap adjust to 1 for fast start
      elif 110 < self.standstill_fault_reduce_timer and CS.cruiseGapSet != 1.0 and self.kisa_cruisegap_auto_adj:
    ...
    # reset lead distnce after the car starts moving
      ...
      btn_signal = self.KCC.update(CS, sm=self.sm)
      ...
      if self.kisa_cruisegap_auto_adj:
        # gap restore
        if self.switch_timer > 0:
          self.switch_timer -= 1
        elif self.dRel > 15 and self.vRel*3.6 < 5 and self.cruise_gap_prev != CS.cruiseGapSet and self.cruise_gap_set_init:
          can_sends.append(hyundaican.create_clu11(self.packer, self.frame, CS.clu11, Buttons.GAP_DIST, self.CP))
          self.cruise_gap_adjusting = True
          self.resume_cnt += 1
          if self.resume_cnt >= int(randint(4, 5) * 2):
            self.resume_cnt = 0
            self.switch_timer = int(randint(20, 25) * 2)
        elif self.cruise_gap_prev == CS.cruiseGapSet and CS.cruiseGapSet != 1.0:
          self.cruise_gap_set_init = False
          self.cruise_gap_prev = 0
          self.cruise_gap_adjusting = False
        else:
    ...
    if self.CP.sccBus == 2 and self.longcontrol:
      if self.frame % 2 == 0:
  ...
  def create_canfd_msgs(self, apply_steer_req, apply_torque, set_speed_in_units, accel, stopping, hud_control, CS, CC, apply_angle, lkas_max_torque, actuators):
    ...
    if self.CP.adrvControl:
      can_sends.extend(hyundaicanfd.create_steering_messages_adrv(self.packer, self.CP, self.CAN, apply_steer_req, apply_torque,
                                                            apply_angle, lkas_max_torque, self.frame, CS.lfa_alt_info, CS.mdps_info, CS.lfa_info, CS.csw_info, CS.adrv_161_info))
    else:
    ...
    if self.CP.adrvControl:
      ...
      can_sends.extend(hyundaicanfd.create_ccnc_messages(self.CP, self.packer, self.CAN, self.frame, CC, CS, hud_control,
                                                         left_lane_warning, right_lane_warning, self.canfd_debug, self.MainMode_ACC_trigger, self.LFA_trigger, self.hdp_use))
      if self.frame % 2 == 0:
        can_sends.append(hyundaicanfd.create_acc_control_scc_adrv(self.packer, self.CAN, CC.enabled, self.accel_last, accel, stopping, CC.cruiseControl.override,
      ...
      if (self.frame - self.last_button_frame) * DT_CTRL > self.refresh_time and CS.acc_active: # 0.25
        ...
        if standstill and (self.last_lead_distance == 0 or self.last_lead_distance > CS.lead_distance):
          ...
          if self.CP.flags & HyundaiFlags.CANFD_ALT_BUTTONS:
            ...
            self.last_lead_distance = 0
            self.standstill_status_canfd = False
        elif standstill and self.kisa_cruisegap_auto_adj and 1.0 not in (CS.cruiseGapSet, CS.DistSet):
        ...
        elif self.kisa_variablecruise or self.try_early_stop and not resume_on:
          self.cruise_gap_adjusting = False
          self.cruise_gap_set_init = False
          self.standstill_res_button = False
          self.auto_res_starting = False
  ...
  def create_common_msgs(self, CS, CC, lat_active, stopping, set_speed_in_units, new_torque):
    ...
    if self.cc_timer > 100:
      ...
      if self.CP.lateralTuning.which() == 'torque' and self.live_torque_params:
