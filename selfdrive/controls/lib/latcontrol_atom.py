    ...
    if not active:
      ...
      if self.multi_lateral_method == LaMethod.SPEED_LOWDT:
        if 2 in self.multi_lat_spdMethod:
          lqr_output_torque, lqr_desired_angle, lqr_log  = self.LaLqr.update( active, CS, VM, params, last_actuators, steer_limited, desired_curvature, desired_curvature_rate, llk )
        if 3 in self.multi_lat_spdMethod:
          toq_output_torque, toq_desired_angle, toq_log  = self.LaToq.update( active, CS, VM, params, last_actuators, steer_limited, desired_curvature, desired_curvature_rate, llk )
        if 1 in self.multi_lat_spdMethod:
          ind_output_torque, ind_desired_angle, ind_log  = self.LaInd.update( active, CS, VM, params, last_actuators, steer_limited, desired_curvature, desired_curvature_rate, llk )
        if 0 in self.multi_lat_spdMethod:
          pid_output_torque, pid_desired_angle, pid_log  = self.LaPid.update( active, CS, VM, params, last_actuators, steer_limited, desired_curvature, desired_curvature_rate, llk )
        ...
        selected = 4
        output_torque, desired_angle  =  self.method_angle( active, CS, VM, params, last_actuators, steer_limited, desired_curvature, desired_curvature_rate, llk )
      elif self.multi_lateral_method == LaMethod.SPEED_INTERP:
        selected = 4
        output_torque, desired_angle  =  self.method_speed( active, CS, VM, params, last_actuators, steer_limited, desired_curvature, desired_curvature_rate, llk )
