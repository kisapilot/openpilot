...
class LatControlATOM(LatControl):
  ...
  def method_angle(self, active, CS, VM, params, steer_limited, desired_curvature, llk):  # angle
    steer_ang = abs(CS.steeringAngleDeg)
    output_torque0, desired_angle0, log0  = self.lat_fun0( active, CS, VM, params, steer_limited, desired_curvature, llk )
    output_torque1, desired_angle1, log1  = self.lat_fun1( active, CS, VM, params, steer_limited, desired_curvature, llk )
    output_torque2, desired_angle2, log2  = self.lat_fun2( active, CS, VM, params, steer_limited, desired_curvature, llk )
  ...
  def method_speed(self, active, CS, VM, params, steer_limited, desired_curvature, llk):  # speed
    ...
    if not active:
      ...
      if self.multi_lateral_method == LaMethod.SPEED_LOWDT:
        if 2 in self.multi_lat_spdMethod:
          lqr_output_torque, lqr_desired_angle, lqr_log  = self.LaLqr.update( active, CS, VM, params, steer_limited, desired_curvature, llk )
        if 3 in self.multi_lat_spdMethod:
          toq_output_torque, toq_desired_angle, toq_log  = self.LaToq.update( active, CS, VM, params, steer_limited, desired_curvature, llk )
        if 1 in self.multi_lat_spdMethod:
          ind_output_torque, ind_desired_angle, ind_log  = self.LaInd.update( active, CS, VM, params, steer_limited, desired_curvature, llk )
        if 0 in self.multi_lat_spdMethod:
          pid_output_torque, pid_desired_angle, pid_log  = self.LaPid.update( active, CS, VM, params, steer_limited, desired_curvature, llk )
        ...
        selected = 4
        output_torque, desired_angle  =  self.method_angle( active, CS, VM, params, steer_limited, desired_curvature, llk )
      elif self.multi_lateral_method == LaMethod.SPEED_INTERP:
        selected = 4
        output_torque, desired_angle  =  self.method_speed( active, CS, VM, params, steer_limited, desired_curvature, llk )
